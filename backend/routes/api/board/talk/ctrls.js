...
boardSchema.commentIds = [{ type: mongoose.Schema.Types.ObjectId, ref: 'TalkComment' }]
commentSchema.boardId = { type: mongoose.Schema.Types.ObjectId, ref: 'Talk', index: true, required: true }
...
exports.del = (req, res) => {
  ...
  Talk.findOne({ _id: _id })
    .then((r) => {
      cp = r;
      return TalkComment.remove({ _id: { $in: r.commentIds } });
    })
    .then(() => {
