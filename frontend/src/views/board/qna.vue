...
export default {
  ...
  data () {
    return {
      ...
      // 부드러운 화면 이동을 위해서 추가
      // 글 작성과 글 삭제는 테이블 행 개수가 바뀌기 때문에 테이블의 갱신이 자연스러우나 나머지는 그렇지 않다.
      row: {},
      rowComment: {}
    }
  },
  ...
  methods: {
    ...
    list (ctx) {
      ...
      this.sortDesc = ctx.sortDesc
      this.isBusy = true
      const p = this.$axios.get('http://localhost:3000/api/board/qna', {
        params: {
          draw: (this.draw += 1),
          search: this.filter,
          skip: this.setSkip,
    ...
    read (r) {
      ...
      this.isBusy = true
      this.$axios.get(`http://localhost:3000/api/board/qna/${_id}`)
        .then((res) => {
          if (!res.data.success) throw new Error(res.data.msg)
    ...
    add (evt) {
      evt.preventDefault()
      this.$axios.post('http://localhost:3000/api/board/qna', this.form)
        .then((res) => {
          if (!res.data.success) throw new Error(res.data.msg)
    ...
    mod () {
      this.$swal({
        ...
        .then((sv) => {
          ...
          return this.$axios.put('http://localhost:3000/api/board/qna', this.form)
        })
        .then((res) => {
          if (!res.data.success) throw new Error(res.data.msg)
    ...
    del (v) {
      this.$swal({
        ...
        .then((sv) => {
          if (!sv) throw new Error('')
          return this.$axios.delete('http://localhost:3000/api/board/qna', {
            params: { _id: v._id }
          })
        })
        .then((res) => {
    ...
    addComment (evt) {
      evt.preventDefault()
      this.$axios.post('http://localhost:3000/api/board/qna/comment', this.formComment)
        .then((res) => {
          if (!res.data.success) throw new Error(res.data.msg)
          // 부드러운 화면 위해
    ...
    modifyComment () {
      this.$swal({
        ...
        .then((res) => {
          ...
          return this.$axios.put('http://localhost:3000/api/board/qna/comment', this.formComment)
        })
        .then((res) => {
          if (!res.data.success) throw new Error(res.data.msg)
    ...
    deleteComment (r, cmt, i) {
      this.$swal({
        ...
        .then((res) => {
          if (!res) throw new Error('')
          return this.$axios.delete('http://localhost:3000/api/board/qna/comment', {
            params: { _id: cmt._id }
          })
        })
        .then((res) => {
